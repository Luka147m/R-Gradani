...
function extendedAnalysis(event: React.MouseEvent<HTMLHeadingElement>, key: number, naslovAnalize: string, naslovSkupaPodataka: string, datumKreiranja: Date, zadnjaIzmjena: Date | undefined, opis: string, izdavac: string, kategorije: string[], vidljivost: "javno" | "privatno", odobreni: boolean, tocke: string[]) {
    console.log(event)
    console.log(key)
...
function BasicAnaliza({key, naslovAnalize, naslovSkupaPodataka, datumKreiranja, zadnjaIzmjena, opis, izdavac, kategorije, vidljivost, odobreni, tocke }: BasicAnalizaProps) {
    return (
        <div className={odobreni ? "odobrena-basic-analiza basic-analiza" : "neodobrena-basic-analiza basic-analiza"} onClick={(event) => extendedAnalysis(event, key, naslovAnalize, naslovSkupaPodataka, datumKreiranja, zadnjaIzmjena, opis, izdavac, kategorije, vidljivost, odobreni, tocke)}>
            <h2>{naslovAnalize}</h2>
